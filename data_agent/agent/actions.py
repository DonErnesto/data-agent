...
class Action:
    def __init__(
        ...
        self.parameters = convert_to_openai_function(pydantic_base_model)["parameters"]
...
def compare_similarity_column_joined_on_key(
    path_df_prev: str, path_df_curr: str, column_name: str
) -> str:
    ...
    both_joined_frac_diff = (
        joined_df.loc[both_joined_idx, f"{column_name}_prev"]
        != joined_df.loc[both_joined_idx, f"{column_name}_curr"]
    ).sum() / number_joined
